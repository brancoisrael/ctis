...
<div class="container" style="background-color: white; margin-top: 0px;">
    ...
    <nav aria-label="breadcrumb" style="margin-top: 0px;">
        <ol class="breadcrumb" style="background-color: white !important;">
            <li class="breadcrumb-item"><a href="${pageContext.request.contextPath}/credenciado/listar">Inicio</a></li>
					...
					<div class="form-group row">
						<div class="col-sm-9">
							<select onchange="processaTipoGuia()" class="form-control" name="idTipoGuia" id="idTipoGuia">
								<s:iterator value="tiposGuiaLaboratorial">
					...
					<div class="card" style="margin-bottom: 30px;">
						...
						<div class="card-body">
							<div class="row">
		               			<div class="col-8">
									<!-- <div class="form-group row">
					                    <label for="inputPaciente" style="text-align: end;" class="col-sm-3 col-form-label">Especialidade:</label>
					                    <div class="col-sm-4">
		               				...
		               				<table class="table" id="procedimentoTable" style="width: 100%; font-size: 12px;">
										<thead>
											<tr>
												...
												<th style="border-top: none;">Quantidade</th>
	...
	<div class="modal fade" id="novoProcedimentoModal" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
	    <div class="modal-dialog modal-lg" role="document">
	        			...
	        			<div class="card" style="border-style: dashed !important;">
		        			<div class="card-body">
			        			...
			        			<div class="row">
			        				...
			        				<div class="col-6">
			        					<strong>Quantidade: </strong><input type="number" id="quantidadeProcedimento" name="quantidadeProcedimento" min="1" max="50"> 
			        				</div>
			        			</div>
		        			</div>
	        			</div>
	        ...
	        <div class="modal-footer">
	            ...
	            <button type="button" onclick="fecharModal()" class="btn btn-secondary" data-dismiss="modal">Fechar</button>
	        </div>
	        </div>
	    </div>
	</div>
...
function addProcedimentoDTO(procedimentoMedico, quantidade, valor) {
	...
	procedimentoMedico.valor = valor.replace(".", "").replace(",", ".");
	
	var procedimento = {
			codigoProcedimento: procedimentoMedico.codigoProcedimento,
			quantidade: procedimentoMedico.quantidade,
...
function removeProcedimentoDTO(codigoProcedimento, quantidade ) {
	//console.warn(codigoProcedimento);
	//console.warn(quantidade);
	$.each(listaProcedimentoDTO, function(i) {
	    if (listaProcedimentoDTO[i].codigoProcedimento == codigoProcedimento && listaProcedimentoDTO[i].quantidade == quantidade) {
	    	listaProcedimentoDTO.splice(i,1);
	        return false;
	    }
	});
	$.each(listaProcedimentos, function(i) {
		console.warn(listaProcedimentos[i]);
	    if (listaProcedimentos[i].codigoProcedimento == codigoProcedimento && listaProcedimentos[i].quantidade == quantidade) {
...
function processaCalculoTotal() {
	...
	//console.warn(listaProcedimentos);
...
function adicionaProcedimentoTable(procedimentoMedico, quantidade, valor) {
	...
	cols += '<td>';
	cols += '<a href="javascript:void(0)" onclick="removerPocedimento(this,'+procedimentoMedico.codigoProcedimento+', '+quantidade+')"><i class="fas fa-trash-alt" data-toggle="modal"  data-placement="top" title="Excluir" style="color:red; cursor: pointer;"></i></a>';
	cols += '</td>';	
	newRow.append(cols);
...
function selecionaProcedimentoModal(procedimento) {
	...
	$.getJSON( "listaProcedimentoJson?codigoProcedimento="+procedimento, function(data) {
		if (data.ret != "0") {
			...
			$('#quantidadeProcedimento').val(data.quantidade);
		} else {
			$("#mensagemRow").show();
		}
