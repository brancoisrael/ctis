...
<div  class="container">
	...
	<div class="container"
		...
		<nav aria-label="breadcrumb" style="margin-top: 0px;">
			<ol class="breadcrumb" style="background-color: white !important;">
				...
				<li class="breadcrumb-item">Guias Médica</li>
			</ol>
		</nav>
		<!-- breadcrumb -->
	</div>
	<div class="container"
		...
		<div class="card" style="margin-bottom: 30px;">
			<div class="card-header">Informações da Credenciada</div>
			<div class="card-body">
				<table style="width: 100%; font-size: 12px;">
					...
					<tr>
						<td><strong>Nome do Usuário: </strong>${usuario.nome}</td>
						<td><strong>Número do Contrato: </strong>${credenciado.numeroContrato}</td>
					</tr>
					<tr>
						<td><strong>Código do Credenciado: </strong>${credenciado.id}</td>
				...
				<div class="card text-white bg-primary" style="width: 100%; box-shadow: 1px 2px 2px 0 rgba(119, 151, 178, 0.16);">
					<div class="row no-gutters">
						...
						<div class="col-md-9 align-self-center">
							<div class="card-body">
								<p class="card-text">Total de Guias emitidas no mês.</p>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="col-3">
				<div class="card text-white bg-success" style="width: 100%;">
					<div class="row no-gutters">
						...
						<div class="col-md-9 align-self-center">
							<div class="card-body">
								<p class="card-text">Total de Guias Atualizadas no mês.</p>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="col-3">
				<div class="card text-white bg-warning"
					...
					<div class="row no-gutters">
						...
						<div class="col-md-9 align-self-center">
							<div class="card-body">
								<p class="card-text">Total de Guias Em Atualização.</p>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="col-3">
				<div class="card text-white bg-danger"
					...
					<div class="row no-gutters">
						...
						<div class="col-md-9 align-self-center">
							<div class="card-body">
								<p class="card-text">Total de Guias Não Autorizados.</p>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="row" style="margin-bottom: 20px;">
					<div class="col-12" style="text-align: end;">
						...
						<a href="${pageContext.request.contextPath}/guia-clinica-laboratorial/frmSetupNovo">
							<button type="button" class="btn btn-primary"><i class="fas fa-plus"></i> Nova Guia Médica</button>
						</a>
						<!-- <a href="${pageContext.request.contextPath}/guia-odontologica/frmSetupNovo">
							<button type="button" class="btn btn-primary"><i class="fas fa-plus"></i> Guia Odontológica</button>
						</a>-->
					</div>				
				</div>
				<div class="row">
...
function pessuisarGuiaMedica(pag) {
	var urlBasePesquisar = "${pageContext.request.contextPath}";
	pesquisarAndRedirect(pag, urlBasePesquisar);
}
function geraTabelaDados(data) {
	...
	if (data.length > 0) {
		data.forEach(function(objeto){
			...
			html += '  <a href="javascript:void(0)" onclick="preparaModal('+objeto.guia.numeroGuia +',' + objeto.guia.anoExercicio + ' , ' + objeto.guia.tipoGuia.id + ', '+urlBase+')"><i class="fas fa-eye" data-toggle="modal"  data-placement="top" title="Visualizar" style="cursor: pointer; font-size: 24px;color: black;"></i></a>';                            
			html += '  <a href="${pageContext.request.contextPath}/guia-clinica-laboratorial/frmSetupEditar?guiaPK.numeroGuia='+objeto.guia.numeroGuia+'&guiaPK.anoExercicio='+objeto.guia.anoExercicio+'&guiaPK.tipoGuia.id='+objeto.guia.tipoGuia.id+'"><i class="fas fa-pen" data-toggle="modal"  data-placement="top" title="Editar" style="cursor: pointer; font-size: 24px;color: black;"></i></a>';
	        html += ' 	</td>';
	        html += '</tr>';
		});
	} else {
