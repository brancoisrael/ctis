...
@Namespace("/guias-odontologica")
@ResultPath(value = "/")
@ParentPackage(value = "default")
public class GuiaOdontologicaListAction extends ActionSupport {
	...
	public String doListarJson() {
		...
		try {
			...
			this.guiaProcedimentoDashBoardDTO = GuiaProcedimentoOdontoBusiness.getInstance().findByParamsNative(params, pagination);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String doInfoGuiaJson() {
		...
		try {
			...
			CustoGuiaDTO custoGuiaDTO = GuiaProcedimentoOdontoBusiness.getInstance().getCustosGuia(guiaGerenciada);
	...
	@Action(value = "despesa-json-odonto",
	...
	@Action(value = "imprimir-json-odonto",
            results = { @Result(name = "success", type = "stream",
                params = { "contentType", "application/octet-stream",
                            "inputName", "inputStream",
                            "bufferSize", "1024",
	...
	public String downloadPDF() {
		try {
				RelatorioSistema relatorio = RelatorioSistema.GUIA_ODONTOLOGICA_PDF;
				...
				this.uploadFileName = "Guia_Odontologica.pdf";
				byte[] buffer = RelatorioBusiness.getInstance().getPdfGuiaOdonto(filtros, relatorio);
				inputStream = new ByteArrayInputStream(buffer);
			}catch (Exception e) {
				return "error";
		}
	...
	@Action(value = "imprimir-json-odonto-tela",
            results = { @Result(name = "success", type = "stream",
	...
	@Action(value = "listar", results = { @Result(name = "success", location = "/credenciada/guias-odontologica/guia-listar.jsp"),
	...
	public String doListar() {
		...
		try {
			...
			this.guiaProcedimentoDashBoardDTO = GuiaProcedimentoOdontoBusiness.getInstance().findByParamsNative(params, pagination);
			//totais cards
			this.totalGuiaEmtidaMes = GuiaBusiness.getInstance().getTotalGuiasAnoMes(YearMonth.from(LocalDate.now()), idCredenciada, ClassificacaoTipoGuia.O);
			this.totalGuiasAutorizadaMes = GuiaBusiness.getInstance().getTotalGuiasAutorizadas(YearMonth.from(LocalDate.now()), idCredenciada,  ClassificacaoTipoGuia.O);
			this.totalGuiasEmAutorizacaoMes = GuiaBusiness.getInstance().getTotalGuiasEmAutorizacao(YearMonth.from(LocalDate.now()), idCredenciada,  ClassificacaoTipoGuia.O);
			this.totalGuiasCanceladasMes = GuiaBusiness.getInstance().getTotalGuiasCanceladas(YearMonth.from(LocalDate.now()), idCredenciada,  ClassificacaoTipoGuia.O);
			//totais cards
