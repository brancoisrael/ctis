...
public class AdministradorAction extends ActionSupport {
	...
	@Action(value = "listar", results = { @Result(name = "success", location = "/adminstrador/guias/guia-listar.jsp"),
	...
	public String doSalvar() {
		...
		try {
			Guia guiaGerenciada = GuiaBusiness.getInstance().getBean(guiaPK);
			guiaGerenciada.setSituacao(situacao);
			guiaGerenciada.setJustificativa(justificativa);
			
			ret = GuiaBusiness.getInstance().alterar(guiaGerenciada);
		} catch (Exception e) {
			return "error";
		}
		return "success";
