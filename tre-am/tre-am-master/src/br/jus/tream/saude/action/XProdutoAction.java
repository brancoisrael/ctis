...
public class XProdutoAction extends ActionSupport {
	private static final long serialVersionUID = 1L;
	private List<XProduto> lstProduto;
...
//			try {
//					this.produto = XProdutoDAOImpl.getInstance().getBean(this.id);
...
//			}
			 int ret = XProdutoDAOImpl.getInstance().inserir(this.produto);
			 if (ret==0) {
...
//						XProduto p = new XProduto();
//						p = XProdutoDAOImpl.getInstance().getBean(this.produto.getId());
//						this.produto.setPicture(p.getPicture());
					}
					 int ret = XProdutoDAOImpl.getInstance().alterar(this.produto);
					 if (ret==0) {
	...
	public String doListarJson() {
		...
		try {
			this.lstProduto = XProdutoDAOImpl.getInstance().listar();
		} catch (Exception e) {
	...
	public List<XProduto> getLstProduto() {
		return lstProduto;
	}

	public void setLstProduto(List<XProduto> lstProduto) {
