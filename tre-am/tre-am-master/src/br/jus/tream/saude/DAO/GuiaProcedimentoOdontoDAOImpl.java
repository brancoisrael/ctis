...
public class GuiaProcedimentoOdontoDAOImpl implements GuiaProcedimentoOdontoDAO {
	
	private DAO<GuiaProcedimentoOdonto> dao = new DAO<GuiaProcedimentoOdonto>(GuiaProcedimentoOdonto.class);

	private static GuiaProcedimentoOdontoDAO db;
	
	private Mapeador mapeador = new Mapeador();

	public static GuiaProcedimentoOdontoDAO getInstance() {
		if (db == null) {
			db = new GuiaProcedimentoOdontoDAOImpl();
		}
		return db;
	}

	@Override
	public GuiaProcedimentoOdonto findById(Long id) {
	...
	public List<GuiaProcedimentoOdonto> findAll() {
		List<GuiaProcedimentoOdonto> lista = new ArrayList<GuiaProcedimentoOdonto>();
		EntityManager em = EntityManagerProvider.getInstance().createManager();
		try {
			Query query = em.createQuery("Select gp FROM GuiaProcedimentoOdonto gp");
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<GuiaProcedimentoOdonto> findAll(Pagination pagination) {
		List<GuiaProcedimentoOdonto> lista = new ArrayList<GuiaProcedimentoOdonto>();
		EntityManager em = EntityManagerProvider.getInstance().createManager();
		try {
			StringBuilder sqlString = new StringBuilder("Select gp FROM GuiaProcedimentoOdonto gp ")
	...
	public int count() throws Exception {
		...
		EntityManager em = EntityManagerProvider.getInstance().createManager();
		try {
			StringBuilder sqlString = new StringBuilder("Select count(gp.id) FROM GuiaProcedimentoOdonto gp ");
	...
	public List<GuiaProcedimentoOdonto> findByParams(GuiaProcedimentoParamsDTO params, Pagination pagination) {
		...
		try {
			...
			Query query = em.createQuery(sqlString.toString(), GuiaProcedimentoOdonto.class)
	...
	/**
	 * Respons�vel por os valores aos par�metros na consulta
	 * @param params
	 * @param query
	 */
	...
	/**
	 * Respons�vel por adicionar os par�metros na consulta
	 * @param params que ser�o adicionados
	 * @return query de acordo com os par�metros
	 * @throws IOException
	 */
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<GuiaProcedimentoOdonto> findByGuia(GuiaPK guiapk) {
		List<GuiaProcedimentoOdonto> lista = new ArrayList<GuiaProcedimentoOdonto>();
		EntityManager em = EntityManagerProvider.getInstance().createManager();
		try {
			StringBuilder sqlString = new StringBuilder("Select gp FROM GuiaProcedimentoOdonto gp ")
	...
	public List<GuiaProcedimentoDashBoardDTO> findByParamsNative(GuiaProcedimentoParamsDTO params, Pagination pagination)
		...
		try {
			StringBuilder sqlb = new StringBuilder(LeitorArquivo.getInstancia()
					.lerMemorizar(ConsultaSistema.LISTAGEM_GUIA_PROCEDIMENTO_ODONTO_DASHBOARD.getQuery()));
			String sql = buildQueryListagemDashboard(params, sqlb);
			Query query = em.createNativeQuery(sql.toString())
...
//				Integer autorizado = obj[3] != null ? new Integer(obj[3].toString()) : null;
				guia.addProcedimento(new ProcedimentoDTO(codProcedimento, codTabela, descricao, null));
			}
		}
	}
	...
	private String buildQueryProcedimentosPorGuia() throws IOException {
		StringBuilder sql_procedimento = new StringBuilder(LeitorArquivo.getInstancia()
				.lerMemorizar(ConsultaSistema.LISTAGEM_PROCEDIMENTO_ODONTO_POR_GUIA_DASHBOARD.getQuery()));
	...
	/**
	 * Respons�vel por adicionar os par�metros na consulta nativa
	 * @param params que ser�o adicionados
	 * @return query de acordo com os par�metros
	 * @throws IOException
	 */
	...
	public int removerByGuia(GuiaPK guiaPk) throws Exception {
		...
		try {
			...
			em.createQuery("Delete from GuiaProcedimentoOdonto gp "
